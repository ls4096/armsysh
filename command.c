...
static int command_process_internal(unsigned char* cmd_str, char process_type);
static unsigned char* is_pipe_cmd(unsigned char* cmd_str);
static int process_second_command(unsigned char* cmd_str);
...
const char* command_tab_complete(const char* cmd, unsigned int cmd_len, unsigned int* match_count)
{
	...
	unsigned int cmd_num = sizeof(CMDS) / sizeof(const char*);
...
static bool begins_with_cmd(const char* str, const char* cmd)
{
	const unsigned int cmd_len = strlen(cmd);
...
static void pc_sys_info()
{
	...
	uint32_t ticks = timer_get_tick_count();
...
static void pc_sleep(const char* cmd_str)
{
	...
	const unsigned int nlen = strlen(cmd_str) - strlen(CMD_SLEEP) - 1;
	unsigned int mul = 1;
	for (i = 1; i < nlen; i++)
	{
		mul *= 10;
...
static void pc_sp_mon_info()
{
	unsigned int* buckets = sp_mon_get_buckets();
	...
	for (i = 0; i < SP_MON_NUM_BUCKETS; i++)
	{
		...
		sprintf(buf,
			"0x%08x-0x%08x: %s%u\r\n",
			i * (1 << SP_MON_BUCKET_SIZE_BITS) + (uint32_t)(&_sstack),
			(i + 1) * (1 << SP_MON_BUCKET_SIZE_BITS) - 1 + (uint32_t)(&_sstack),
			bar,
			buckets[i]);
		serial_write(buf, strlen(buf));
	}
